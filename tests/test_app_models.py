...
def test_article(my_db: SQLAlchemy, article_id, num_scored_topics, num_scored_labels):
    ...
    assert_rel_collection_belongs_to_entity(
        a.scored_topics, num_scored_topics, lambda x: x.article_id, a.id
    )
    assert_rel_collection_belongs_to_entity(
        a.scored_labels, num_scored_labels, lambda x: x.article_id, a.id
    )


def assert_rel_collection_belongs_to_entity(
    entity_rel_collection: list, length: int, get_id: Callable[[_E], _R], id_value: _R
):
