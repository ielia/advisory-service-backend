...
def set_up_common_routes(bp: Blueprint, cls: type[Model, SerializerMixin], history_cls: type[Model, SerializerMixin], ref_id_field_name: str):
    @bp.post('/')
    ...
    def list_objs() -> tuple[Response, int]:
        objs = cls.query.all()
        return jsonify(
            {snakecase(cls.__Plural__): [obj.to_dict() for obj in objs], 'result': 'ok',
