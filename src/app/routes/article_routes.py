...
def follow_article(id_value: int) -> tuple[Response, int]:
    ...
    if article.following:
        return jsonify({'article': article.to_dict(), 'result': 'ok',
                        'message': f"Article {article.id} already being followed"}), 200  # or error 409
    ...
    with db.session.no_autoflush:
        ...
        preexisting_labels = db.session.query(Label).filter(Label.text.in_([l.text for l in new_labels])).all()
        for p_label in preexisting_labels:
            idx, dup_label = next((l for l in enumerate(new_labels) if l[1].text == p_label.text), None)
            del new_labels[idx]
    ...
    return jsonify({'article': article.to_dict(), 'generated_tags': [tag.to_dict() for tag in tags],
                    ...
                    'generated_labels': [{**tl.label.to_dict(), 'topic_id': tl.topic_id, 'weight': tl.weight}
                                         for tl in topic_labels],
                    'result': 'ok', 'message': f"Article {article.id} now being followed"}), 201


@article_bp.delete('/<int:id_value>/follow')  # a DELETE does not have a response body, but if we switch to POST it will
def unfollow_article(id_value: int) -> tuple[Response, int]:
    article = Article.query.get_or_404(id_value)
    if not article.following:
        return jsonify({'article': article.to_dict(), 'result': 'ok',
                        'message': f"Article {article.id} was not being followed"}), 204  # or error 409

    tags: list[Tag] = db.session.query(Tag).filter(Tag.article_id == article.id).options(
    ...
    with db.session.no_autoflush:
        ...
        for tag in tags:
            ...
            if not topic.is_global and (len(topic.tags) == 1 or tags_to_delete.issuperset(topic.tags)):
        ...
        for topic in topics_to_delete:
            for tl in topic.topic_labels:
                ...
                label = tl.label
                if len(label.topic_labels) == 1 or topics_to_delete.issuperset([tl.topic for tl in label.topic_labels]):
                    db.session.delete(label)

        db.session.commit()

    return jsonify({'article': article.to_dict(), 'result': 'ok',
                    'message': f"Article {article.id} is not being followed any longer"}), 204
...
def generate_ai_summary(id_value: int) -> tuple[Response, int]:
    ...
    if article.ai_summary is None:
        ...
        db.session.commit()
        return jsonify(
            {'article': article.to_dict(), 'result': 'ok', 'message': f"Article {article.id} summarized"}), 201
    else:
        return jsonify({'article': article.to_dict(), 'result': 'ok',
