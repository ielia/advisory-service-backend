...
def add_to_topic(id_value: int, topic_id: int) -> tuple[Response, int]:
    ...
    topic_label = TopicLabel(label_id=label.id, label=label, topic_id=topic.id, topic=topic, weight=weight, enabled=data.get('enabled', True))
    db.session.add(topic_label)
    db.session.commit()
    return jsonify({'topic_label': topic_label.to_dict(), 'result': 'ok', 'message': f"TopicLabel {topic_label.id} created"}), 201
