...
def add_label(id_value: int) -> tuple[Response, int]:
    ...
    nulls = [
        p[0]
        for p in [("hypothesis", hypothesis), ("text", text), ("weight", weight)]
        if p[1] is None
    ]
    if len(nulls) > 0:
    ...
    return jsonify(
        {
            ...
            "result": "ok",
            "message": f"Label {label.id} created under Topic {topic.id} created",
        }
    ), 201
        ...
        return jsonify(
            {
                "topic": topic.to_dict(),
                "result": "ok",
                "message": f"Topic {topic.id} is already global",
            }
        ), 200
    else:
        new_topic = Topic(name=topic.name, is_global=True)
        old_topic_labels = TopicLabel.query.filter(
            TopicLabel.topic_id == topic.id
        ).all()
        new_topic.topic_labels = [
            TopicLabel(
                label=tl.label, topic=new_topic, weight=tl.weight, enabled=tl.enabled
            )
            for tl in old_topic_labels
        ]
        ...
        return jsonify(
            {
                "topic": topic.to_dict(),
                "result": "ok",
                "message": f"Topic {topic.id} is now global",
            }
        ), 201
    ...
    return jsonify(
        {
            "topic": topic.to_dict(),
            "labels": [
                {"label": tl.label.to_dict(), "weight": tl.weight}
                for tl in topic.topic_labels
            ],
            "result": "ok",
            "message": f"Topic {topic.id} found",
        }
    ), 200
...
def remove_label(id_value: int, label_id: int) -> tuple[Response, int]:
    ...
    topic_label = next(
        (tl for tl in topic.topic_labels if tl.label.id == label_id), None
    )
    ...
    return jsonify(
        {
            "topic": topic.to_dict(),
            "result": "ok",
            "message": f"Topic {topic.id} is now enabled",
        }
    ), 201
...
def disable_topic(topic_id: int) -> tuple[Response, int]:
    ...
    affected_articles: list[Article] = (
        Tag.query.filter(Tag.topic_id == topic_id)
        .join(Article)
    ...
    ArticleTie.query.filter(
        ArticleTie.original_article_id.in_(affected_articles_ids)
    ).delete()
