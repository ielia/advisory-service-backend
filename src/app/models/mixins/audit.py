...
class AuditMixin(AuditMetaMixin):
    ...
    def create_history_model(cls: db.Model) -> type[db.Model]:
        ...
        ref_id_col_names: list[str] = [
            f"{snakecase(cls.__singular__)}_{cn}" for cn in model_id_col_names
        ]
        ...
        audit_columns = {
            ...
            "change_type": Column(
                String(max(len(ct.name) for ct in ChangeType)), nullable=False
            ),
            "change_date": Column(
                DateTime,
                default=lambda: datetime.now(timezone.utc),
                server_default=SDUTCNow(),
                nullable=False,
            ),
            "change_user_id": Column(String(100), nullable=False),
            "change_reason": Column(Text),
            **{
                ref_id_name: Column(id_col.type, nullable=False)
                for id_col, ref_id_name in zip(model_id_cols, ref_id_col_names)
            },
        }
        ...
        cast(
            ...
            and_(
                *[
                    ...
                    for id_name, ref_id_name in zip(
                        model_id_col_names, ref_id_col_names
                    )
                ]
            ),
        )
            ...
            relationship(
                ...
                primaryjoin=cast(
                    ...
                    and_(
                        *[
                            getattr(cls, id_name)
                            == foreign(getattr(history_cls, ref_id_name))
                            for id_name, ref_id_name in zip(
                                model_id_col_names, ref_id_col_names
                            )
                        ]
    ...
    @staticmethod
    def _log_history(
        history_cls: type[db.Model],
        model_id_col_names: list[str],
        ref_id_col_names: list[str],
