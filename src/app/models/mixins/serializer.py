...
class SerializerMixin:
    ...
    def to_dict(self, *expand: str) -> dict[str, Any]:
        def _build_rel_expand_tree_node(*expand: str) -> dict[str, list[str | None]]:
            expand_split: list[list[str]] = [e.split('.', 1) for e in sorted(set(expand)) if e is not None]
        ...
        def _normalize_value(c_type, val: Any, *exp: str) -> bool | float | int | str | list | dict | None:
        ...
        data = {c.name: _normalize_value(c.type, getattr(self, c.name)) for c in self.__table__.columns}

        if expand:
            expand_dict = _build_rel_expand_tree_node(*expand)
            mapper = class_mapper(self.__class__)
            for rel_name in expand_dict.keys():
                rel_prop: RelationshipProperty = mapper.relationships.get(rel_name, None)
                if rel_prop is None:
                    continue  # skip invalid relation names
                value = getattr(self, rel_name)
                data[rel_name] = _normalize_value(rel_prop.argument, value, *expand_dict[rel_name])
